	...
	<div class="container" >
		...
		<h5 class="display-5">
			<form class="form-inline" method="POST" action="<c:url value='/conseiller/client/infos' />"">
	  			<input type="hidden" name="client" value="${ requestScope.message.client.id }" />
	  			<button class="btn btn-link" style="color: gray;">${ requestScope.message.client.nom } ${ requestScope.message.client.prenom }</button>
	  		</form>
		</h5>
		<br/>
		<div>
			...
			<div class="row" style="margin-bottom: 10px;">
				<div class="card col-md-8 ${ requestScope.message.destinataire == 'conseiller' ? 'offset-md-4 text-right text-white bg-info' : ''}">
				  <div class="card-header">
				  	<c:if test="${ requestScope.message.destinataire == 'client' }">
				  		Moi
				  	</c:if>
				  	<c:if test="${ requestScope.message.destinataire == 'conseiller' }">
				  		<form class="form-inline" method="POST" action="<c:url value='/conseiller/client/infos' />" style="justify-content:flex-end; ">
				  			<input type="hidden" name="client" value="${ requestScope.message.client.id }" />
				  			<button class="btn btn-link" style="color: white;">${ requestScope.message.client.nom } ${ requestScope.message.client.prenom }</button>
				  		</form>
				  	</c:if>
				  </div>
				  <div class="card-body">
			...
			<c:forEach items="${ requestScope.message.reponses }" var="reponse" >
				<div class="row" style="margin-bottom: 10px;">
					<div class="card col-md-8 ${ reponse.destinataire == 'conseiller' ? 'offset-md-4 text-right text-white bg-info' : '' }">
					  <div class="card-header">
		...
		<div>
			<div class="container">
				...
				<form method="POST" action="<c:url value='/conseiller/messagerie/message' />" class="form-inline col-md-12" style="justify-content: center;">
